...
struct TriggerCronJobIntent: AppIntent {
    ...
    static var description = IntentDescription(
        LocalizedStringResource(
            ...
            defaultValue: "Runs the selected Palvia cron job now. Does not open the app, so automations can fire while the phone is locked."
        )
    )
    ...
    func perform() async throws -> some IntentResult {
        ...
        _ = await CronJobRunner.runOne(jobId: job.id, container: PalviaModelContainer.shared)
        return .result()
    }
}
