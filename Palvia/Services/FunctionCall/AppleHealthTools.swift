...
struct AppleHealthTools {
    ...
    private func checkWriteAuth(_ id: HKQuantityTypeIdentifier) -> String? {
        ...
        if !pm.isHealthSharingAuthorized(for: type) {
            return "[Error] Write access denied for this data type. Please enable it in Settings > Health > Data Access & Devices > Palvia."
        }
        return nil
    }
    ...
    func writeDietaryEnergy(arguments: [String: Any]) async -> String {
        ...
        if let note = arguments["note"] as? String, !note.isEmpty { metadata["PalviaNote"] = note }
