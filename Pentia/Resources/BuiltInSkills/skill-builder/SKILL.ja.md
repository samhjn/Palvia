...
description: fs.* ブリッジ経由で Pentia スキルを直接作成・編集します。スキャフォールディング、ツール／スクリプトのジェネレータ、検証、リファレンス例を含みます。
---
# スキルビルダー（Skill Builder）

ユーザーから Pentia スキルの作成・修正・フォークを依頼されたときに使用してください。
Pentia のスキルは `/skills/<slug>/` 配下のディレクトリで、`fs.*` ブリッジを使って
...
5. **検証。** `validate_skill(slug=...)` は自動リロードで使われるのと同じ
   ...
   warning は参考情報です。
6. **インストール。** `install_skill(name=<frontmatter の name>)`。Pentia は
