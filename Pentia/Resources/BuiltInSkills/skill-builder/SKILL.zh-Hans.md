---
display_name: 技能构建器
description: 通过 fs.* 桥接直接创作和编辑 Pentia 技能。包含脚手架、工具/脚本生成器、校验以及示例参考。
---
# 技能构建器（Skill Builder）

当用户要求你创建、修改或复刻一个 Pentia 技能时使用本技能。Pentia 的技能
...
6. **安装。** `install_skill(name=<frontmatter 中的 name>)`。Pentia 会
