...
final class ShareViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NSLog("[PalviaShare] viewDidLoad — containerAvailable=%@ snapshotAgents=%d",
    ...
    private func handleSelection(agent: AgentSnapshotEntry) {
        ...
        NSLog("[PalviaShare] handleSelection agent=%@ inputItems=%d",
        ...
        Task { [weak self] in
            ...
            await MainActor.run {
                guard let handoffId else {
                    NSLog("[PalviaShare] stager returned nil — nothing to share")
                    self.finish(withError: ShareL10n.errorNothingToShare)
                    return
                }
                NSLog("[PalviaShare] stager produced handoffId=%@", handoffId.uuidString)
    ...
    private func openHostApp(agentId: UUID, handoffId: UUID) {
        ...
        // staged handoff the next time the user opens Palvia.
    ...
    /// open Palvia manually, since iOS won't let a share extension foreground
    /// its host app.
